...
namespace TarsEngine::Graphics
{
	...
	namespace SimpleDraw
	{
		...
		void AddSphere(uint32_t slices, uint32_t rings,float rad,const Math::Vector3& origin,const Color& color);

		void AddGroundPlane(float size, const Color& color);
		void AddGroundCircle(uint32_t slices,float rad, const Math::Vector3& origin,const Color& color);
