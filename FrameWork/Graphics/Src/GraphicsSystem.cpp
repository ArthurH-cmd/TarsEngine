...
namespace
{
	...
	Core::WindowMessageHandler sMessageHandler;
}
...
LRESULT CALLBACK GraphicsSystem::GraphicsSystemMessageHandle(HWND win, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	return sMessageHandler.FowardMessage(win, msg, wParam, lParam);
}
...
{
	ASSERT(mD3DDDevice == nullptr && mImmediantContext == nullptr, "[GraphicsSystem] Terminate must be called.");
}
...
void GraphicsSystem::Initialize(HWND window, bool fullscreen)
{
	...
	sMessageHandler.Hook(window, GraphicsSystemMessageHandle);
}

void GraphicsSystem::Terminate()
{
	sMessageHandler.Unhook();
	SafeRelease(mDepthStencilView);
	SafeRelease(mDepthStencilBuffer);
	SafeRelease(mRenderTargetView);
	SafeRelease(mSwapChain);
	SafeRelease(mImmediantContext);
	SafeRelease(mD3DDDevice);
}

void GraphicsSystem::BeginRender()
{
	mImmediantContext->OMSetRenderTargets(1, &mRenderTargetView, mDepthStencilView);
	mImmediantContext->ClearRenderTargetView(mRenderTargetView,(FLOAT*) (&mClearColor));
	mImmediantContext->ClearDepthStencilView(mDepthStencilView, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);
}
...
void GraphicsSystem::ToggleFullscreen(HWND window)
{
	BOOL fullscreen;
...
{
	mImmediantContext->OMSetRenderTargets(0, nullptr, nullptr);
	...
	if (width != GetBackBufferWidth() || height != GetBackBufferHeight())
	{
		...
		ASSERT(SUCCEEDED(hr), "GraphicSystem: Failed to accses swap chain view");
		mSwapChain->GetDesc(&mSwapChainDesc);
	}

	ID3D11Texture2D* backBuffer = nullptr;
	hr = mSwapChain->GetBuffer(0, IID_PPV_ARGS(&backBuffer));
	ASSERT(SUCCEEDED(hr), "GraphicSystem: Failed to get backBuffer");

	hr = mD3DDDevice->CreateRenderTargetView(backBuffer, nullptr, &mRenderTargetView);
	SafeRelease(backBuffer);
	ASSERT(SUCCEEDED(hr), "GraphicSystem: Failed to created render Target");
	...
	hr = mD3DDDevice->CreateTexture2D(&depthDesc, nullptr, &mDepthStencilBuffer);
...
{
	mImmediantContext->RSSetViewports(1, &mViewport);
}
